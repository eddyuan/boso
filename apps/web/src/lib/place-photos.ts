...
import { fetchPhotoBytes, MAX_PHOTOS_PER_PLACE, type PhotoRef } from "./places-google";
...
export type PhotoFill = { places: number; photos: number; requests: number };
...
export async function materializePhotos(placeIds: string[]): Promise<PhotoFill> {
  if (placeIds.length === 0) return { places: 0, photos: 0, requests: 0 };
  ...
  return { places: pending.length, photos, requests };
}
